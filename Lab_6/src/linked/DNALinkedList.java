...
public class DNALinkedList {
	...
	// the nodes at the start of that chain. If the target appears multiple
	// times
	...
	// firstExtractedNode is not the head, and lastExtractedNode is not the
	// tail.
	...
	public DNALinkedList extract(Node<Character> firstExtractedNode, Node<Character> lastExtractedNode) {
		// Find nodes just before and just after the chain to be extracted.
		// Assume these
		// aren't null.
	...
	public void reverse() {
		// Swap next and prev of every node. Caution: in your loop, you won't be
		// able
		// to advance n by setting n = n.next(). Why? How should you advance n?
		Node<Character> n = head;
		while (n != null) {
			Node<Character> oldPrev = n.getPrev();
	...
	public void insert(DNALinkedList insertMe, Node<Character> insertionPoint) {
		...
		insertMe.tail.setNext(insertionPoint.getNext());
		insertionPoint.setPrev(insertMe.tail);
	}

	// Removes sequence matching transposon, reverses it, and inserts it back
	// into
	...
	public void transpose(String transposon, String target) {
		...
		if (firstNodeOfTransposon == null)
			throw new IllegalArgumentException("Cannot find traspon");
		...
		// Find last node of transposon. You'll need several lines. Set a
		// variable to the first node
		// of the transposon, then do a loop where for every char in the
		// transposon, you set the variable
		// to its "next".
		...
		insert(transposonList, lastNodeOfTransposon.getNext());
	}

	public static void main(String[] args) {
		String chromosome = "ATTAGGGGG";
		DNALinkedList list = new DNALinkedList(chromosome);
		System.out.println("original: " + list);
		String transposon = "TT";
		String target = "G";
